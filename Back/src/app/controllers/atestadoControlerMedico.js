...
class AtestadoController {
    ...
    async show(req, res) {
        try {
            ...
            const atestadoData = await atestadoModel.findAll({ where: { medicoFK: medico } });
    ...
    async barChart(req, res) {
        try {
            const rg  = req.params.rg; // Substitua pelo valor de RG desejado
            ...
            const results = await atestadoModel.findAll({
                ...
                where: {
                    medicoFK: rg, // Adicione a condição medicoFK = rg aqui
                },
                group: ['ano', 'mes'],
                order: ['ano', 'mes'],
            });
    ...
    async pieChart(req, res) {
        try {
            const rg = req.params.rg; // Obtém o valor de 'rg' da URL
            ...
            const results = await atestadoModel.findAll({
                ...
                where: {
                    medicoFK: rg // Adiciona a condição medicoFK = rg aqui
                },
